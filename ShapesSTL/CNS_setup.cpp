...
CNS::variableSetUp ()
{
    ...
    cnt++; set_scalar_bc(bc,phys_bc); bcs[cnt] = bc; name[cnt] = "rhoY";
    cnt++; set_scalar_bc(bc,phys_bc); bcs[cnt] = bc; name[cnt] = "rho_e";
    ...
    // Gradient of reactant mass fraction vector (to track flame location)
    ...
    var_names_gy[2]="dYdz";
#endif
    derive_lst.add("dY",IndexType::TheCellType(), AMREX_SPACEDIM,
                   var_names_gy, cns_derYderiv, grow_box_by_one);
    derive_lst.addComponent("dY",desc_lst,State_Type,URHO,NUM_STATE);
