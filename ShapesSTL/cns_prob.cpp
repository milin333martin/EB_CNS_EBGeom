...
extern "C" {
	...
	// Get the location of the von-Neumann shock (in metres) (this MUST be in inputs file)
        pp.get("xsh", CNS::h_prob_parm->xsh);
	...
	pp.query("inflow_time", CNS::h_prob_parm->inflow_time);
	
        // RADIUS OF HIGH TEMPERATURE BURNT GAS (IN SI UNITS)
        pp.query("radhitemp", CNS::h_prob_parm->radhitemp);
        
        // Specialized reactant region (Cylinder)
        pp.query("yCenter", CNS::h_prob_parm->yCenter);
        pp.query("zCenter", CNS::h_prob_parm->zCenter);
        pp.query("rCyl", CNS::h_prob_parm->rCyl);
        pp.query("LCyl", CNS::h_prob_parm->LCyl);
        
        // Blast ball
        pp.query("sphere_radius", CNS::h_prob_parm->sphere_radius);
        ...
        pp.get("zreflo", CNS::h_prob_parm->zreflo);
        pp.get("zrefhi", CNS::h_prob_parm->zrefhi);
